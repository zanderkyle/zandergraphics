...
class JFormFieldCkcolor extends JFormField
{
	...
	protected function getInput()
	{
		...
		else
		{
			...
			$html = '<div style="display:inline-block;vertical-align:top;margin-top:4px;width:20px;"><img src="' . $this->getPathToElements() . '/images/color.png" style="margin-right:5px;" /></div>';
			$html .= '<input type="text" name="' . $this->name . '" id="' . $this->id . '"' . ' value="'
				. htmlspecialchars($color, ENT_COMPAT, 'UTF-8') . '"' . $hint . $class . $position . $control
				. $readonly . $disabled . $required . $onchange . $autocomplete . $autofocus . ' style="width:103px;"/>';
			return $html;
		}
	}
