 ...
 */
function CreativecontactformBuildRoute( &$query )
{
	static $items;
	...
	if (!$items) {
		$component	= JComponentHelper::getComponent('com_creativecontactform');
		$menu	= $app->getMenu();
	...
	if (is_array($items))
	{
		...
		if (!$itemid) {
			foreach ($items as $item)
			{
				...
				if (isset($item->query['view']) && $item->query['view'] == 'Creativecontactform'
		...
		if (!$itemid) {
			foreach ($items as $item)
			{
				if (isset($query['view']) && $query['view'] == 'creativecontactform' && isset($item->query['view']) && $item->query['view'] == 'creativecontactform')
				{
 ...
 * @return	array
 */
function CreativecontactformParseRoute( $segments )
{
	$vars = array();
