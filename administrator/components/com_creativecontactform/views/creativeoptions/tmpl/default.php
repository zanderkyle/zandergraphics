	...
	<fieldset id="filter-bar">
		...
		<div class="filter-select fltrt">
			...
			<select name="filter_category_id" class="inputbox" onchange="this.form.submit()">
				<option value=""><?php echo JText::_('COM_CREATIVECONTACTFORM_SELECT_CATEGORY');?></option>
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th width="5%">
					<?php echo JHtml::_('grid.sort', 'COM_CREATIVECONTACTFORM_NUM_ANSWERS', 'num_answers', $listDirn, $listOrder); ?>
				</th>
				<th width="1%">
		...
		foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td class="center">
					<?php echo JHtml::_('creativefield.featured', $item->featured, $i, true); ?>
				</td>
				<td align="center">
					<a href="<?php echo JRoute::_('index.php?option=com_creativecontactform&task=creativetemplate.edit&id='.(int) $item->template_id); ?>">
...
<?php endif;?>
		...
		<table class="table table-striped" id="articleList">
			<thead>
				<tr>
					...
					<th>
						<?php echo JHtml::_('grid.sort', 'COM_CREATIVECONTACTFORM_LABEL', 'sp.name', $listDirn, $listOrder); ?>
					</th>
					<th width="20%">
						<?php echo JHtml::_('grid.sort', 'COM_CREATIVECONTACTFORM_FORM', 'form', $listDirn, $listOrder); ?>
					</th>
					<th width="10%">
						<?php echo JHtml::_('grid.sort', 'COM_CREATIVECONTACTFORM_FIELD', 'field', $listDirn, $listOrder); ?>
					</th>
					<th width="1%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'JGRID_HEADING_ID', 'sp.id', $listDirn, $listOrder); ?>
					</th>
			...
			foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							<a href="<?php echo JRoute::_('index.php?option=com_creativecontactform&task=creativeoption.edit&id='.(int) $item->id); ?>">
								<?php echo $this->escape($item->name); ?>
							</a>
						</div>
					</td>
					<td align="small hidden-phone">
						<a href="<?php echo JRoute::_('index.php?option=com_creativecontactform&task=creativeform.edit&id='.(int) $item->template_id); ?>">
							<?php echo $item->form; ?>
						</a>
					</td>
					<td class="nowrap has-context">
		...
		<input type="hidden" name="view" value="creativeoptions" />
		<input type="hidden" name="task" value="" />
		<input type="hidden" name="boxchecked" value="0" />
		<input type="hidden" name="filter_order" value="<?php echo $listOrder; ?>" />
		<input type="hidden" name="filter_order_Dir" value="<?php echo $listDirn; ?>" />
