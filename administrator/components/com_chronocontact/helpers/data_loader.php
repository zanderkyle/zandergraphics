			...
			foreach($matches[0] as $field){
				...
				if(!empty($name_attr[2])){
					...
					if(!in_array($name, $skipped) AND !is_null($value)){
						...
						$value = htmlspecialchars(self::_escape_regex($value)); //escape any special (") or regex chars ($)
			...
			foreach($matches[0] as $field){
				...
				if(!in_array($name, $skipped) AND !is_null($value)){
					...
					foreach($matched_options[0] as $matched_option){
						...
						if(isset($matched_option_value[2]) AND in_array($matched_option_value[2], (array)$value)){
							$updated_option = preg_replace('/<option/i', '<option selected="selected"', $matched_option);
						}elseif(isset($matched_option_value[2]) AND !in_array($matched_option_value[2], (array)$value)){
