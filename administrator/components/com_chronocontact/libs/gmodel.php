...
class GModel extends Model {
	...
	function beforeSave(&$data, &$params, $mode){
		if(isset($data['params']) AND is_array($data['params'])){
			$p_obj = new Parameter($data['params']);
			$data['params'] = $p_obj->toString();
		}
		if(isset($data['rules']) AND is_array($data['rules'])){
			$p_obj = new Parameter($data['rules']);
			$data['rules'] = $p_obj->toString();
		}
		if(isset($data['pages']) AND is_array($data['pages'])){
			$p_obj = new Parameter($data['pages']);
			$data['pages'] = $p_obj->toString();
		}
		if(isset($data['extras']) AND is_array($data['extras'])){
			$base_string = new Base64('');
			foreach($data['extras'] as $k => $v){
				$base_string->set($k, $v);
			}
			$data['extras'] = $base_string->toString();
		}
				...
				foreach($models_list as $model){
					if(!empty($data[$k][$model])){
						if(isset($data[$k][$model]['extras'])){
							...
							$data[$k][$model]['extras'] = $extras->extract();
						}
						if(isset($data[$k][$model]['params'])){
							$params = new Parameter($data[$k][$model]['params']);
							$data[$k][$model]['params'] = $params->toArray();
						}
						if(isset($data[$k][$model]['rules'])){
							$rules = new Parameter($data[$k][$model]['rules']);
							$data[$k][$model]['rules'] = $rules->toArray();
						}
						if(isset($data[$k][$model]['pages'])){
							$pages = new Parameter($data[$k][$model]['pages']);
							$data[$k][$model]['pages'] = $pages->toArray();
						}
