...
class Lang {
		...
		if(empty(self::$lang)){
			...
			if(Base::getConfig('detect_language', 0) == 1){
				...
				if(!empty($chunks) AND strlen($chunks[0]) == 5){
					$lang = str_replace('-', '_', $chunks[0]);
					$cutsom_lang = '\GCore\Locales\\'.Str::camilize($lang).'\Lang';
	...
	public static function load($path = ''){
		if(!empty($path) AND !in_array($path, self::$loaded)){
			...
			$lang_class = '\GCore'.$path.'\Locales\\'.Str::camilize(str_replace('-', '_', Base::getConfig('site_language', 'en-gb'))).'\Lang';
			$cutsom_lang = '\GCore'.$path.'\Locales\\'.Str::camilize(self::$lang).'\Lang';			
			if(class_exists($cutsom_lang)){
				$lang_class = $cutsom_lang;
			}
			if(!class_exists($lang_class)){
			...
			$lang_class = new \ReflectionClass($lang_class);
			self::$translations = array_merge((array)self::$translations, $lang_class->getConstants());
			self::$loaded[] = $path;
			return true;
		}
		return false;
