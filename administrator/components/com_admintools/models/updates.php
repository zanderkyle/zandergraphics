...
Your site has determined that there is an updated version of Admin Tools
...
ENDBODY;
    ...
    private function getLastSend()
    {
        ...
        $query = $db->getQuery(true)
                    ...
                    ->where($db->qn('key').' = '.$db->q('admintools_autoupdate_lastsend'));
    ...
    private function setLastSend()
    {
        ...
        if($count)
        {
            $query = $db->getQuery(true)
                        ...
                        ->set($db->qn('value').' = '.$db->q($now->toSql()))
                        ->where($db->qn('key').' = '.$db->q('admintools_autoupdate_lastsend'));
        ...
        {
            $data = (object) array(
                'key'   => 'admintools_autoupdate_lastsend',
                'value' => $now->toSql()
            );
