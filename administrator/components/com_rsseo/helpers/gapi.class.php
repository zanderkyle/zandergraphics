...
class gapi
{
   ...
   */
  public function requestAccountData($start_index=1, $max_results=100)
  {
    $params = array();
	$params['start-index'] = $start_index;
	$params['max-results'] = $max_results;
	
	$apiKey = rsseoHelper::getConfig('analytics_key');
	
	if (!empty($apiKey))
		$params['key'] = $apiKey;
	
	$response = $this->httpRequest(gapi::account_data_url, $params, null, $this->generateAuthHeader());
  ...
  protected function accountObjectMapper($xml_string)
  {
    ...
    $account_root_parameters['startDate'] = !empty($google_results) ? strval($google_results->startDate) : $startDate;
    $account_root_parameters['endDate'] = !empty($google_results) ? strval($google_results->endDate) : $endDate;
  ...
  protected function authenticateUser($email, $password)
  {
    ...
    {
      throw new Exception('GAPI: Failed to authenticate user.');
    }
