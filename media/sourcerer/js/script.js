...
(function ($) {
	...
	sourcerer_init = function () {
		...
		if (string.search(preg_quote(sourcerer_tag_characters[0] + sourcerer_syntax_word)) != -1) {
	...
	sourcerer_insertText = function () {
		...
		var start_tag = preg_quote(t_start + t_word) + '.*?' + preg_quote(t_end);
		var end_tag = preg_quote(t_start + '/' + t_word + t_end);
